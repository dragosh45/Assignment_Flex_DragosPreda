...
class TenantResolver:
    ...
    def resolve_tenant_from_token(token_payload: dict) -> Optional[str]:
        ...
        """
        # Tenant authorization comes from server-controlled claims, never user metadata.
    ...
    async def resolve_tenant_id(user_id: str, user_email: str, token: Optional[str] = None) -> Optional[str]:
        ...
        if token:
            try:
                ...
                return None
        # Tokenless callers already authenticated these two seeded local users.
        return {
            ("user-sunset", "sunset@propertyflow.com"): "tenant-a",
            ("user-ocean", "ocean@propertyflow.com"): "tenant-b",
        }.get((user_id, user_email))
